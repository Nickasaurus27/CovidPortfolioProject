...
--Creating Views 
	--For reference and perhaps some later analysis
...
-- Removing income bracket records as well from 'location' column
-- European Union is part of Europe
SELECT location, SUM(new_deaths) total_death_count
FROM covid_deaths
...
-- 4.
-- Same as above but with the 'cal_date' column added
SELECT location, population, cal_date, MAX(total_cases) highest_infection_count,
MAX(ROUND((total_cases / population)*100,2)) percent_pop_infected
FROM covid_deaths
WHERE continent IS NOT NULL
GROUP BY 1,2,3
